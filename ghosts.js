...
export function spawnGhosts(level) {
    ...
    map.forEach((row, y) => row.forEach((v, x) => {
        if (v !== 1 && (x > 5 || y > 5)) safe.push({x, y});
    }));
    for (let i = 0; i < Math.min(2 + level, 4); i++) {
        let pos = safe[Math.floor(Math.random() * safe.length)];
        ghosts.push({ x: pos.x, y: pos.y, vX: pos.x, vY: pos.y, color: colors[i % 4], speed: 0.08, dirX: 0, dirY: 0, dead: false });
    }
}

export function updateGhosts(lives, level, score) {
    if (berserker.active) { berserker.timer--; if (berserker.timer <= 0) berserker.active = false; }
    for (let g of ghosts) {
        if (g.dead) continue;
        if (Math.abs(g.x - g.vX) < 0.1 && Math.abs(g.y - g.vY) < 0.1) {
            g.vX = g.x; g.vY = g.y;
            let moves = [{dx:1,dy:0},{dx:-1,dy:0},{dx:0,dy:1},{dx:0,dy:-1}].filter(m => map[Math.round(g.y + m.dy)]?.[Math.round(g.x + m.dx)] !== 1);
            if (moves.length > 1) moves = moves.filter(m => m.dx !== -g.dirX || m.dy !== -g.dirY);
            let c = moves[Math.floor(Math.random() * moves.length)];
            if(c) { g.dirX = c.dx; g.dirY = c.dy; g.x += g.dirX; g.y += g.dy; }
        }
        g.vX += (g.x - g.vX) * 0.1; g.vY += (g.y - g.vY) * 0.1;
        if (Math.hypot(g.vX - pacman.vX, g.vY - pacman.vY) < 0.6) {
            if (berserker.active) { g.dead = true; score.value += 200; setTimeout(() => { g.dead = false; g.x = 7; g.y = 7; }, 3000); }
            else { lives.value--; resetPlayer(); spawnGhosts(level); return; }
        }
    }
}
...
export function drawGhosts(ctx, tileSize, offsetX, offsetY) {
    for (let g of ghosts) {
        ...
        ctx.lineTo(x + s * 0.25, y + s * 0.75); // Punta 2
        ...
        ctx.fillStyle = "blue"; // Pupilas arriba
