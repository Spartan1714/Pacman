...
export function updateGhosts(lives, level, score) {
    ...
    for (let g of ghosts) {
        ...
        // IA de movimiento centrada en celdas
...
export function drawGhosts(ctx, tileSize, offsetX, offsetY) {
    for (let g of ghosts) {
        ...
        // Ojos: Pupilas azules mirando hacia ARRIBA (como en tu imagen)
        ctx.fillStyle = "white";
        ctx.beginPath();
        ctx.arc(x + s*0.35, y + s*0.4, s*0.15, 0, 7);
        ctx.arc(x + s*0.65, y + s*0.4, s*0.15, 0, 7);
        ctx.fill();
        ctx.fillStyle = "blue";
        ctx.beginPath();
        ctx.arc(x + s*0.35, y + s*0.32, s*0.07, 0, 7);
        ctx.arc(x + s*0.65, y + s*0.32, s*0.07, 0, 7);
        ctx.fill();
    }
}