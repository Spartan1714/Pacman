...
function checkWin() { return !map.some(row => row.includes(2)); }
...
function drawMap() {
    ...
    map.forEach((row, y) => row.forEach((tile, x) => {
        ...
        else if (tile === 2) { ctx.fillStyle = "white"; ctx.beginPath(); ctx.arc(rx+tileSize/2, ry+tileSize/2, tileSize/8, 0, 7); ctx.fill(); }
    }));
}

function gameLoop() {
    if (lives.value > 0) {
        updatePlayer(score); updateGhosts(lives, currentLevel);
        if (checkWin()) { currentLevel++; resetLevel(); }
        drawMap(); drawGhosts(ctx, tileSize, offsetX, offsetY); drawPlayer(ctx, tileSize, offsetX, offsetY);
        ctx.fillStyle = "white"; ctx.font = "20px Arial";
        ctx.fillText(`SCORE: ${score.value} LIVES: ${lives.value} LVL: ${currentLevel}`, offsetX, offsetY - 10);
        requestAnimationFrame(gameLoop);
    } else {
        ctx.fillStyle = "red"; ctx.font = "50px Impact"; ctx.fillText("GAME OVER", canvas.width/2 - 100, canvas.height/2);
    }
}
